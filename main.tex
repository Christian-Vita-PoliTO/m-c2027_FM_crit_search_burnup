...
% let floats share their page with more text: with six floats in ten pages the
% default parameters produce nearly empty float pages
...
\title{Extending the Fission Matrix Criticality Search to Fuel Burnup: a Burnup Correction Ratio for Heat-Pipe Micro Reactors}
...
\addAuthor{Fausto Franceschini}{2}

\addAffiliation{1}{Politecnico di Torino, Dipartimento Energia, Torino, Italy}

\addAffiliation{2}{Westinghouse Mangiarotti, v. Timavo 59, Monfalcone, 34074, Italy}

\Abstract{%
The fission matrix (FM) method turns the criticality search of a heat-pipe micro reactor (HPMR) into the eigenproblem of a matrix, interpolated from a database of FMs tallied by a Monte Carlo (MC) code at different core configurations. In previous work the database was built over the fuel temperature and the control-drum angle, and the critical drum position at any power is found in a fraction of a second. This paper adds fuel burnup to the method without adding a third axis to the database. The burnup effect is factorized out of the interpolated matrix through a multiplicative Burnup Correction Ratio (BCR): the ratio between the matrices tallied at a burnup snapshot and at the fresh state, at fixed conditions. One low-statistics depletion of the VTB HPMR model at nominal power gives the compositions; twelve independent full-statistics transport calculations, restarted from it, tally the snapshot matrices at about 66 CPU\,h each. The statistical uncertainty of both databases is propagated to every result with an ensemble of fifty database realizations, in which each coefficient is perturbed independently within its tally uncertainty. Over a 66-month cycle at \SI{2}{MW} the critical drum angle moves from \ang{62} to \ang{12}; each time step takes less than a second on a single processor of a personal computer, the ensemble runs for the uncertainty budget included. Reference Serpent 2 calculations at the states returned by the algorithm give $k_{\rm eff}$ within $-59$ to $-5$ pcm of unity, with the same bias as at beginning of life. The three assumptions of the factorization -- angle-independent, frozen-temperature and single-history BCR -- are measured with dedicated campaigns and found at or below that residual.
}

\keywords{fission matrix, criticality search, burnup, heat-pipe micro reactor, Monte Carlo}

\shortTitle{Extending the Fission Matrix Criticality Search to Fuel Burnup}
...
\begin{equation}
    ...
    \label{eq:fm}
\end{equation}
at the cost of an $N\times N$ eigenproblem. The matrix depends on the state of the core: temperatures, absorber positions, compositions. The method becomes predictive when a database of tallied matrices covers the states of interest and the matrix at any other state is interpolated from it with a certain granularity.

This work extends to fuel burnup the framework of Ref.~\cite{vita2026fm}. There, a database was built for the VTB HPMR model \cite{ortensi2024hpmr} over two axes, the fuel temperature and the control-drum angle, and a criticality search algorithm was built around it \cite{vita2025fim4ntra}, with a power-to-temperature correlation standing in for the thermal solver. The critical drum angle at any power is obtained in a fraction of a second, with $k_{\rm eff}$ within a few tens of pcm of reference Serpent 2 calculations \cite{leppanen2015serpent}. Burnup was left out. Fuel depletion changes the FM coefficients through the composition, and a cycle analysis needs those changes: the critical drum angle, the remaining excess reactivity, the cycle length. Burnup-dependent FM methods exist, for example in the RAPID code system \cite{pungercic2023brapid} and in the combined FM depletion coupling of Ref.~\cite{he2024combined}. The question here is different: how to add burnup to a database that already spans temperature and drum angle, without multiplying its size.

This paper proposes a factorization. The interpolated matrix is multiplied element-wise by a Burnup Correction Ratio (BCR): the ratio between the matrix tallied at a burnup snapshot and the matrix tallied at the fresh state, under fixed reference conditions. The BCR is tallied once per snapshot time, at a single drum angle and at the nominal temperature state, and applied at every state the search visits. Search, database and verification protocol are unchanged; what is added is the composition axis and the cycle it makes possible. The study is done at the nominal power of \SI{2}{MW}. Nothing in the construction depends on the power: the temperature axis of the database carries the power dependence, so the extension to other power levels is immediate.
...
The database of Ref.~\cite{vita2026fm} (FMDB) has 8 fuel temperature levels and 17 drum angles, from \ang{0} (absorber fully extracted) to \ang{180} (absorber facing the core): 136 criticality calculations. The median relative uncertainty of the coefficients is \SI{9}{\percent}, and each entry costs about 63 CPU\,h (Table~\ref{tab:cost}). The matrix at any state is obtained by bivariate linear interpolation, $\tilde a_{ij}(T_{f,i},\theta)$: the fuel temperature of the target cell $i$ selects the position on the temperature axis, the drum angle $\theta$ the position on the angle axis.

\subsection{The search}

The search of Ref.~\cite{vita2026fm} is not changed by the burnup extension. Two nested loops surround the interpolated matrix. The inner loop finds the neutronic--thermal equilibrium at fixed drum angle: the matrix is interpolated at the current temperatures, Eq.~\eqref{eq:fm} is solved, the temperatures are updated from the new fission source, and the loop repeats until the temperature profile changes by less than \SI{0.5}{\percent}.

No thermal solver is coupled to the fission matrix. The fuel temperature of cell $i$ follows from the fission source through the power-to-temperature correlation of Ref.~\cite{vita2026fm},
\begin{equation}
    T_i \;=\; \frac{P}{P_{\rm nom}}
    \left[\;\frac{F_i - F_{\min}}{F_{\max} - F_{\min}}\;\Delta T \;+\; \left(T_{f,\min} - T_0\right)\right] \;+\; T_0 ,
    \label{eq:t}
\end{equation}
with $T_0 = \SI{300}{K}$ the cold state of the database, and $T_{f,\min} = \SI{823}{K}$ and $\Delta T = \SI{51}{K}$ the parameters that reproduce, at nominal power, the \SIrange{823}{874}{K} range of the reference multiphysics solution of the same core. Three properties of this mock-up matter for what follows. It is local and instantaneous: each cell is mapped on its own, with no conduction and no heat-pipe dynamics. It is normalized: the coldest and the hottest cell sit at the two ends of the interval by construction, so what Eq.~\eqref{eq:t} provides is the spatial redistribution of the temperature -- which assemblies are hot -- and not the absolute spread, which is fixed a priori at each power. And it is shared by the workflow and by the reference calculations against which the workflow is verified, which impose the same profile through the multiphysics interface: the verification therefore measures the fission matrix interpolation and the BCR, and cannot see the thermal model itself.

The outer loop is a bisection on the drum angle in $[\ang{0},\ang{180}]$. For scans, the stopping criterion is deterministic, $|k_{\rm eff}-1| < 1$ pcm. When the database uncertainty is propagated, as described below, the search stops when the $\pm 1\sigma$ band of the last two iterates contains $k_{\rm eff}=1$: the drum angle is not resolved beyond the statistical resolution of the database. If the core is subcritical with the drums fully extracted, criticality cannot be restored. This guard, added for the present work, defines the end of cycle life.
...
\subsection{The standard database generation burden}
...
\begin{table}[tb]
    ...
    \begin{tabular}{lcc}
        ...
        Depletions (drums out; drums at \ang{33.75} for the history test) & 2 & 2.75 h on 64 cores each \\
        Snapshot matrices at $\theta_{\rm ref}=\ang{0}$ & 12 & 66 CPU\,h each \\
        Reference criticality calculations & 12 & 62 CPU\,h each \\
        Snapshots at \ang{45}, \ang{90}, \ang{135}, \ang{180}; evolved $T$; alt.\ history & $48+1+5$ & 66 CPU\,h each \\
        \midrule
...
\begin{figure}[tb]
    ...
    \begin{tikzpicture}[
        ...
        box/.style   = {draw, rounded corners=1pt, align=center, inner sep=3pt,
                        text width=58mm, minimum height=4.4mm},
        dec/.style   = {draw, diamond, aspect=3.6, align=center, inner sep=0pt,
                        text width=42mm, minimum height=4.4mm},
        io/.style    = {draw, thick, rounded corners=3pt, align=center,
                        inner sep=3pt, text width=58mm},
        ar/.style    = {-{Latex[length=1.6mm]}}]

    \node[io]                       (start) {$t=0$, \; $BU_i = 0$, \; $F$ flat};
    \node[box, below=of start]      (mat)   {$A = \tilde a_{ij}(T_{f,i},\theta)\cdot BCR_{ij}(t)$};
    \node[box, below=of mat]        (th)    {$T_i$ from $F$ (Eq.~\ref{eq:t}); \;
                                             $k_{\rm eff}, F$ from $A\,F = k_{\rm eff}F$};
    \node[dec, below=3mm of th]     (dt)    {$\Delta T < 0.5\%$ ?};
    \node[dec, below=3mm of dt]     (crit)  {$|k_{\rm eff}-1| \le \sigma_k$ twice ?};
    \node[box, below=3mm of crit]   (bu)    {record $\theta^*(t)$, $\sigma_\theta = \sigma_k/|{\rm d}k_{\rm eff}/{\rm d}\theta|$; \\
                                             $BU_i \leftarrow BU_i + P\,\bar F_i\,\Delta t/m_i$};
    \node[dec, below=3mm of bu]     (eol)   {critical with $\theta = 0$ ?};
    \node[io, below=3mm of eol]     (eolx)  {end of cycle life};

    \draw[ar] (start) -- (mat);
    \draw[ar] (mat) -- (th);
    \draw[ar] (th) -- (dt);
    \draw[ar] (dt) -- node[right, pos=0.45] {yes} (crit);
    \draw[ar] (crit) -- node[right, pos=0.45] {yes} (bu);
    \draw[ar] (bu) -- (eol);
    \draw[ar] (eol) -- node[right, pos=0.45] {no} (eolx);

    \draw[ar] (dt.west) -- ++(-5mm,0) node[midway, above] {no} |- (th.west);
    \draw[ar] (crit.east) -- ++(6mm,0) node[midway, above] {no}
              |- (mat.east) node[pos=0.28, right, xshift=1mm] {bisect $\theta$};
    \draw[ar] (eol.west) -- ++(-8mm,0) node[midway, above] {yes}
              |- (mat.west) node[pos=0.28, left, xshift=-1mm] {$t \leftarrow t+\Delta t$};
    \end{tikzpicture}
    \caption{The quasi-static cycle. The inner loop (left return) is the neutronic--thermal equilibrium at fixed drum angle; the outer loop (right return) is the bisection on the drum angle, stopped when the tallied $k_{\rm eff}$ sits inside the $\pm\sigma_k$ band that the fifty database realizations give at that angle; the outermost loop advances the cell-wise burnup and enters the next time step. Burnup enters only through the factor $BCR_{ij}(t)$ in the second box.}
    \label{fig:flow}
\end{figure}
...
Along the cycle $\sigma_k$ grows from 11 pcm at BOL, where the BCR is identically one and only the FMDB contributes, to 30--36 pcm once the snapshot tallies enter the ratio. The corresponding $\sigma_\theta$ grows from \ang{0.14} to \ang{1.3}. It does so not because $\sigma_k$ grows -- it is nearly constant along the cycle -- but because the differential drum worth falls from 78 to 25 pcm per degree as the drums are withdrawn: the same uncertainty on $k_{\rm eff}$ maps onto a wider interval in angle. The linear propagation was verified against fifty full searches per state, one per database realization: the two estimates of $\sigma_\theta$ agree within \SI{3}{\percent}. Perturbing the coefficients independently ignores the correlations among the tallies of one run, so the bands are conservative: the replicate comparisons reported below show that they overestimate the observed scatter by roughly a factor 1.5.
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.87\textwidth]{figures/critical_drum_angle_over_the_cycle_with_maps.pdf}
...
The reactivity with the drums fully extracted, evaluated along the cycle, sets the cycle length at its crossing of unity (\Cref{fig:drumsout}). It starts at 2870 pcm at BOL, a value read from the tallied database and not a free parameter. It decreases by 500 pcm per year over the second year and by 376 pcm per year at the end of the window (307 and 231 pcm per MWd/kg), and 120 pcm are left at 66 months: end of life falls near month 70. The reference points are the snapshot transports themselves, full-statistics calculations at $\theta = \ang{0}$ with the composition of each snapshot. They are tallied at the reference temperature state, while the workflow curve carries the evolved profile, a difference bounded by the frozen-temperature test below. The maps in \Cref{fig:drumsout} show the flattening of the fission source over the cycle: the assembly peaking factor decreases from 1.221 to 1.097, the centre loses \SI{11}{\percent} and the drum-adjacent periphery gains \SI{21}{\percent}. The assembly-wise error of the workflow source against the reference is \SI{0.24}{\percent} on average. Behind this, the depletion output gives \SI{5.75}{\percent} core-average $^{235}$U depletion at 66 months and a $^{239}$Pu share of the fissions reaching \SI{3.6}{\percent}.

\begin{figure}[tb]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/excess_reactivity_with_the_drums_fully_extracted.pdf}
...
% through the local drum worth of 55 pcm per degree, that maximum is 85 pcm, dominated by the tally noise of the additional planes.

\begin{figure}[tb]
    \centering
    \includegraphics[width=0.68\textwidth]{figures/independence_hypothesis_structural_error_measured.pdf}
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.60\textwidth]{figures/interpolation_error_vs_snapshot_grid_density.pdf}
