...
Along the cycle $\sigma_k$ grows from 11 pcm at BOL, where the BCR is identically one and only the FMDB contributes, to 30--36 pcm once the snapshot tallies enter the ratio. The corresponding $\sigma_\theta$ grows from \ang{0.14} to \ang{1.3} at end of life, where the differential drum worth vanishes. Perturbing the coefficients independently ignores the correlations among the tallies of one run, so the bands are conservative: the replicate comparisons of Section~\ref{sec:hyp} show that they overestimate the observed scatter by roughly a factor 1.5.
